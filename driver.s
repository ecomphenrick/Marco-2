...
comeca_infer:                        @é a chamada pelo programa c, depois que todos os dados foram enviados 
    push {r7, r10, lr}               @ c passa o ponteiro virtual em r0, salva em r10/ diferente do mandar_inst que tinha r10 setado globalmente 
    mov  r10, r0                        

    mov  r2, #5                         
    str  r2, [r10, #PIO_DATA_IN]        @instrução start, nao tem endereço nem dado, so opcode op =101 = 5 decimal
    mov  r2, #1                         
    str  r2, [r10, #PIO_SIGNALS]        
    mov  r2, #0                         
    str  r2, [r10, #PIO_SIGNALS]        @mesmo pulso enable 1 pra 0 

comecar_poll:                            @marcador de posição, um beq que usa como destino de desvio 
    ldr  r2, [r10, #PIO_DATA_OUT]       @ ler o valor de 32 bit doi resgistrador data out r10
    tst  r2, #(1 << 4)                  @faz  and entre o valor lido e ox10, nao salva resultado, so aualiza as flags
    beq  comecar_poll                     @ desvia deprendendo da flag estiver ativa. Se done - 0, flag 0 ta ativa,desvia de volta pra comecar poll

    ldr  r0, [r10, #PIO_DATA_OUT]       @le o data out, e coloca em r0 o valor de retorno , o C recebe e aplica oxf pra isolar os bit 0-3 com o digito predito

    pop  {r7, r10, lr} @restaura
    bx   lr             @retorna 


mandar_instrucao: @função mais chamada do drive(confirmação pra instrução)
    push {r7, lr}
    str  r2, [r10, #PIO_DATA_IN]        @escreve a instrução de 32 bits no endereço/ recebe o data in mais ainda nao executa
    mov  r2, #1                         
    str  r2, [r10, #PIO_SIGNALS]        @enable 1 = avisa ao coprocessador que tem instrução nova
    mov  r2, #0                         
    str  r2, [r10, #PIO_SIGNALS]        @enable 0 = obrigatorio, o done so sobe depois que o enable desce se ficar em 1 
poll_fim:                               @trava no poll 
    ldr  r2, [r10, #PIO_DATA_OUT]       
    tst  r2, #(1 << 4)                  
    beq  poll_fim                      
    pop  {r7, lr}
    bx   lr                             @ restaura os registradores e retorna pra função que chamadora

mandar_sem_espera:                      @igual de cima so que sem pop/push e sem o polling/ nao faz bl pra nenhuma outra função
    str  r2, [r10, #PIO_DATA_IN]        
    mov  r2, #1                         @usada so no store_pesos pra op =001, define o endereço onde peso é armazenado 
    str  r2, [r10, #PIO_SIGNALS]        
    mov  r2, #0                         
    str  r2, [r10, #PIO_SIGNALS]        
    bx   lr
